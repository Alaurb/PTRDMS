...
Raw greenhouse panoramas, site maps, and derived archived demonstrations are not redistributed in this public package. They remain under the data owner's control and may be requested from the authors subject to authorization. The reviewed crop images, annotations, fixed splits, and exclusion list are instead published separately as the [Four-Stage Tomato Ripeness Crop Dataset for PTRDMS](https://doi.org/10.5281/zenodo.22822983). The absence of the raw panorama and map materials does not change the stated claim boundary: panorama observations are not ground truth, and metric mapping requires independently validated geometry.
