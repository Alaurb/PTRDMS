...
| Continuous sequence analysis | **Implemented:** `demo.py:run()` writes `processing_mode="offline_batch"`; `ripeness_demo/report.py:write_html_report()` writes the local report | online acquisition and real-time latency are not claimed |
