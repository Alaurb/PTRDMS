...
All crops derived from the same source image were assigned to the same split. The images and annotations are retained by the authors and are available upon reasonable request; this repository publishes the deployed weight and the evaluation record, not the crop corpus.
